...
namespace DapperPerfTest.PerfTest.Query
{
    public class RelatedEntities : PerfTestBase
    {
        ...
        [Benchmark]
        public IReadOnlyCollection<EndangeredCustomer> EFCore5()
        {
            var customerQuery =
                from c in this.EF5Context.Customers
        ...
        [Benchmark]
        public IReadOnlyCollection<EndangeredCustomer> EFCore6()
        {
            return this.EFCore();
        }

        [Benchmark]
        public IReadOnlyCollection<EndangeredCustomer> EFCore6Compiled()
        {
            return this.EFCore();
        }

        private IReadOnlyCollection<EndangeredCustomer> EFCore()
        {
            var customerQuery =
                from c in this.EF6Context.Customers
                let affectedOrders = c.Orders.SelectMany(o => o.OrderDetails.Where(_ => _.ProductId == ProductId))
                where affectedOrders.Any()
                select new EndangeredCustomer
                {
